					   ...
					   S3C_FRAME_BUFFER *fb)
{
	...
	if (sFBVar.xres_virtual != sFBVar.xres || sFBVar.yres_virtual < ulYResVirtual)
	{
		...
		if (res != 0)
		{
			pr_err("%s: fb_set_var failed (Y Offset: %d, Error: %d)\n", __FUNCTION__, fb->yoffset, res);
		}
	}
	else
	{
		...
		if (res != 0)
		{
			pr_err( "%s: fb_pan_display failed (Y Offset: %d, Error: %d)\n", __FUNCTION__, fb->yoffset, res);
		}
	}
...
static S3C_BOOL CreateVsyncWorkQueue(S3C_LCD_DEVINFO *psDevInfo)
{
	...
	{
		pr_err("fail to create vsync_handler workqueue\n");
		return S3C_FALSE;
	}
...
{	
	...
	{
		pr_err("S3C_InstallVsyncISR: Couldn't install system LISR on IRQ %d", VSYNC_IRQ);
		return;
	}
}
...
static S3C_BOOL InitDev(struct fb_info **s3c_fb_Info)
{
	...
	{
		pr_err("Couldn't get framebuffer module\n");
		eError = S3C_FALSE;
		goto errRelSem;
	}
	...
	if (psLINFBInfo->fbops->fb_open != NULL)
	{
		...
		if (res != 0)
		{
			pr_err("Couldn't open framebuffer: %d\n", res);
			eError = S3C_FALSE;
			goto errModPut;
		}
	}
